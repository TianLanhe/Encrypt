#include "Encrypt.h"
#include "include/ThreadPoolFactory.h"
#include "include/ThreadPool.h"
#include <iterator>
#include <algorithm>
#include <string>

using namespace std;

ostream& operator<<(ostream& out,const EncryptData& data){
    if(out && data.m_bValid){
        copy(data.m_data.begin(), data.m_data.end(), ostream_iterator<char>(out));
    }
    return out;
}

istream& operator>>(istream& in,EncryptData& data){
    if(in){
        vector<unsigned char> tmp = vector<unsigned char>(istreambuf_iterator<char>(in),istreambuf_iterator<char>());
        data.m_data.swap(tmp);
        data.m_bValid = true;
    }
    return in;
}

bool EncryptData::Encrypt(const string& pwd){
    if(!m_bValid || pwd.empty())
        return false;

    ThreadPool* pool = ThreadPoolFactory::GetInstance()->GetThreadPool();

    string::size_type pwdlen = pwd.size();
    int threadNum = pool->GetThreadsNum();

    int units = m_data.size() / pwdlen;
    int bytes = m_data.size() % pwdlen;
    int units_one = units / threadNum;
    int units_leave = units % threadNum;

    int units_total = 0;
    int units_real;
    int start,end;
    for(int i=0;i<threadNum;++i){
        units_real = units_one;
        if(i < units_leave)
            ++units_real;

        start = units_total * pwdlen;
        end = units_real * pwdlen;

        units_total += units_real;

        if(i == threadNum - 1)
            end = m_data.size();

        CalcTask* task = new CalcTask(&m_data, start, end);
        task->setPwd(pwd);
        pool->AddTask(task);
    }

    pool->StartTasks();
    pool->WaitForThreads();
    delete pool;

    return true;
}

CalcTask::CalcTask(vec* data,size_type start,size_type end):m_data(data),m_start(start),m_end(end){
    if(!m_data){
        if(m_start)
            m_start = 0;
        if(m_end)
            m_end = 0;
    }else{
        if(m_start >= m_data->size())
            m_start = 0;
        if(m_end > m_data->size())
            m_end = m_data->size();
    }
}

Status CalcTask::Run(){
    CHECK_ERROR(!m_pwd.empty() && m_data);

    string::size_type pwdlen = m_pwd.size();
    string::size_type j = 0;
    for(size_type i = m_start; i != m_end; ++i){
        (*m_data)[i] = (*m_data)[i] ^ m_pwd[j++ % pwdlen];
    }
    return OK;
}





